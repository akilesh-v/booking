...
CREATE TABLE IF NOT EXISTS rooms_master_aud
(
    id                  bigint                            NOT NULL,
    ...
    max_child           numeric                           NOT NULL,
    max_child_age_limit numeric                           NOT NULL,
    CONSTRAINT rooms_master_aud_pkey PRIMARY KEY (id, rev),
    CONSTRAINT rooms_master_aud_revinfo FOREIGN KEY (rev)
        REFERENCES revision (id) MATCH SIMPLE
        ON UPDATE NO ACTION ON DELETE NO ACTION
);

CREATE TABLE IF NOT EXISTS room_price_aud
(
    id                bigint                      NOT NULL,
    rev               integer                     NOT NULL,
    revtype           smallint,
    ...
    extra_adult_price double precision            NOT NULL,
    extra_child_price double precision            NOT NULL,
    base_adult_count  numeric                     NOT NULL,
    CONSTRAINT room_price_aud_pkey PRIMARY KEY (id, rev),
    CONSTRAINT room_price_aud_revinfo FOREIGN KEY (rev)
        REFERENCES revision (id) MATCH SIMPLE
        ON UPDATE NO ACTION ON DELETE NO ACTION
);